#!/usr/bin/env bash
# Combined LAPE pipeline for Gemma-2-2b that runs once with the
# original datasets and once with word-shuffled prompts.
# Each run collects neuron activation counts and then identifies
# language-specific and shared neurons via the plain LAPE algorithm.
...
OUT_DIR="$ROOT/output_lape_shuffling_gemma"
...
run_mode() {
  ...
  echo "=========================================="
  echo "Gemma LAPE Shuffling Pipeline - ${mode_label}"
  echo "Model: $MODEL_ID"
...
echo "All Gemma LAPE shuffling runs completed. Artifacts stored under $OUT_DIR"
