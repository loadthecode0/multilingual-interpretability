...
if [ -z "$EXP_TYPE" ] || [ -z "$LANG" ] || [ -z "$NUMS" ]; then
    echo "Usage: $0 [shuffling|romanization] [lang] [num1 num2 ...]"
    exit 1
fi

for NUM in $NUMS; do
    echo "------------------------------------------------------------------------"
    echo "Running Random Control Gemma Raw: Exp: $EXP_TYPE, Lang: $LANG, Num: $NUM"
    echo "------------------------------------------------------------------------"
